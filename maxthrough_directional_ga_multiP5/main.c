...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    rhs[3] = mxDuplicateArray(prhs[3]);
    rhs[4] = mxDuplicateArray(prhs[4]);
    tempsu = mxGetPr(rhs[0]);
    temppu = mxGetPr(rhs[1]);
    input_values = mxGetPr(rhs[2]);
    tempalpha_su = mxGetPr(rhs[3]);
    tempalpha_pu = mxGetPr(rhs[4]);
    N = input_values[0];
    N_f = input_values[1];
    M = input_values[2];
    B = input_values[3];
    ...
    G_M = input_values[8];
    G_S = input_values[9];
    theta = input_values[10];
    SINRPULT = input_values[11];
    SINRSULT = input_values[12];
   ...
   /* printf("here");
    ...
    /*allocation for alpha_su*/
    
    alpha_su = (double**) malloc((M)*sizeof(double*));
    for(i=0;i<M;i++)
    {
        alpha_su[i]=(double*)malloc((2*M)*sizeof(double));
    }
    ...
    for(i = 0;i<M;i++)
    {
        for(j=M;j<2*M;j++)
        {
            alpha_su[i][j] = tempalpha_su[j*M+i];
            mexPrintf("alpha_su[%d][%d] = %lf\n",i,j,alpha_su[i][j]);
        }
    }
    
    /*allocation of alpha_pu*/
    
    alpha_pu = (double**) malloc((M)*sizeof(double*));
    for(i=0;i<M;i++)
    {
        alpha_pu[i] = (double*)malloc((2*N_f)*sizeof(double));
    }
    for(i=0;i<M;i++)
    {
        for(j=N_f;j<2*N_f;j++)
        {
            alpha_pu[i][j] = tempalpha_pu[j*M+i];
            mexPrintf("alpha_pu[%d][%d] = %lf\n",i,j,alpha_pu[i][j]);
        }
    }
